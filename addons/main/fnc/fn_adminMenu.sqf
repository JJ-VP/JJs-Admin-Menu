...
[] spawn {
	with uiNamespace do {
		...
		hintHeader = missionNamespace getVariable "hintHeader";
		...
		ctrlbtnLoad ctrlAddEventHandler ["ButtonClick", {
			with uiNamespace do {
				...
				if (_index != -1) then {
					...
					hintSilent parseText format ["%1You equiped loadout<br/><t color=""#42ebf4"">%2</t>", hintHeader, _name];
					player setVariable ["hintTimer", 3];
				} else { 
					hintSilent parseText format ["%1You need to select a loadout", hintHeader];
					player setVariable ["hintTimer", 3];
				};
			};
		}];
		...
		ctrlbtnSave ctrlAddEventHandler ["ButtonClick", {
			with uiNamespace do {
				...
				if (_loadoutName != "") then {
					...
					hintSilent parseText format ["%1You created loadout<br/><t color=""#42ebf4"">%2</t>", hintHeader, _loadoutName];
					player setVariable ["hintTimer", 3];
				} else {
					hintSilent parseText format ["%1You need to specify a loadout name", hintHeader];
					player setVariable ["hintTimer", 3];
				};
			};
		}];
		...
		ctrlbtnDelete ctrlAddEventHandler ["ButtonClick", {
			with uiNamespace do {
				...
				if (_index != -1) then {
					...
					profileNamespace setVariable ["jjx_loadouts", _allLoadouts];
					hintSilent parseText format ["%1You deleted loadout<br/><t color=""#42ebf4"">%2</t>", hintHeader, _name];
					player setVariable ["hintTimer", 3];
				} else { 
					hintSilent parseText format ["%1You need to select a loadout", hintHeader];
					player setVariable ["hintTimer", 3];
			...
			with uiNamespace do {
				...
				featuresList = missionNamespace getVariable "featuresList";
				{
					switch(_x select 1) do {
						...
						case "god": { 
							_player = allPlayers select (lbCurSel 1500);
						...
						case "ammo": {
							if ((lbCurSel 1500) != -1) then {
								if (isNil {((allPlayers select (lbCurSel 1500)) getVariable "jjx_ammo")} || {!((allPlayers select (lbCurSel 1500)) getVariable "jjx_ammo")}) then {
									ctrllstFeatures lbAdd format ["%1 - OFF", _x select 0];
									ctrllstFeatures lbSetColor [_forEachIndex, JJAM_admin_red];
									ctrllstFeatures lbSetTooltip [_forEachIndex, _x select 3];
								} else {
						...
						case "rapid": {
							if ((lbCurSel 1500) != -1) then {
								if (isNil {((allPlayers select (lbCurSel 1500)) getVariable "jjx_rapid")} || {!((allPlayers select (lbCurSel 1500)) getVariable "jjx_rapid")}) then {
									ctrllstFeatures lbAdd format ["%1 - OFF", _x select 0];
									ctrllstFeatures lbSetColor [_forEachIndex, JJAM_admin_red];
									ctrllstFeatures lbSetTooltip [_forEachIndex, _x select 3];
								} else {
						...
						case "sway": {
							if ((lbCurSel 1500) != -1) then {
								if (isNil {((allPlayers select (lbCurSel 1500)) getVariable "jjx_sway")} || {!((allPlayers select (lbCurSel 1500)) getVariable "jjx_sway")}) then {
									ctrllstFeatures lbAdd format ["%1 - OFF", _x select 0];
									ctrllstFeatures lbSetColor [_forEachIndex, JJAM_admin_red];
									ctrllstFeatures lbSetTooltip [_forEachIndex, _x select 3];
								} else {
						...
						case "speed": {
							if ((lbCurSel 1500) != -1) then {
								if (isNil {((allPlayers select (lbCurSel 1500)) getVariable "jjx_speed")} || {!((allPlayers select (lbCurSel 1500)) getVariable "jjx_speed")}) then {
									ctrllstFeatures lbAdd format ["%1 - OFF", _x select 0];
									ctrllstFeatures lbSetColor [_forEachIndex, JJAM_admin_red];
									ctrllstFeatures lbSetTooltip [_forEachIndex, _x select 3];
								} else {
						...
						case "mapTP": {
							if ((lbCurSel 1500) != -1) then {
								if (isNil {((allPlayers select (lbCurSel 1500)) getVariable "jjx_mapTP")} || {!((allPlayers select (lbCurSel 1500)) getVariable "jjx_mapTP")}) then {
									ctrllstFeatures lbAdd format ["%1 - OFF", _x select 0];
									ctrllstFeatures lbSetColor [_forEachIndex, JJAM_admin_red];
									ctrllstFeatures lbSetTooltip [_forEachIndex, _x select 3];
								} else {
						...
						case "freeze": {
							if ((lbCurSel 1500) != -1) then {
								if (isNil {((allPlayers select (lbCurSel 1500)) getVariable "jjx_frozen")} || {!((allPlayers select (lbCurSel 1500)) getVariable "jjx_frozen")}) then {
									ctrllstFeatures lbAdd format ["%1 - OFF", _x select 0];
									ctrllstFeatures lbSetColor [_forEachIndex, JJAM_admin_red];
									ctrllstFeatures lbSetTooltip [_forEachIndex, _x select 3];
								} else {
						...
						case "markers": {
							if ((lbCurSel 1500) != -1) then {
								if (isNil {((allPlayers select (lbCurSel 1500)) getVariable "mapMarkers")} || {!((allPlayers select (lbCurSel 1500)) getVariable "mapMarkers")}) then {
									ctrllstFeatures lbAdd format ["%1 - OFF", _x select 0];
									ctrllstFeatures lbSetColor [_forEachIndex, JJAM_admin_red];
									ctrllstFeatures lbSetTooltip [_forEachIndex, _x select 3];
								} else {
		...
		JJAM_execute = {
			...
			if (_feature != -1) then {
				...
				hintSilent parseText format ["%1Please select a feature to execute.", hintHeader];
		...
		while {true} do {
			...
			hintSilent parseText format ["%1 Someone tried to freeze you!<br />Don''t worry, I saved you", hintHeader];
			player setVariable ["hintTimer", 5];
		};
	};
};
...
JJAM_admin_god = {
	...
	if (isDamageAllowed _player) then {
		...
		hintSilent parseText format ["%1You made<br/><t color=""#42ebf4"">%2</t><br />immortal", hintHeader, name _player];
		player setVariable ["hintTimer", 3];
	} else {
		[_player, true] remoteExec ["allowDamage", 2];
		hintSilent parseText format ["%1You made<br/><t color=""#42ebf4"">%2</t><br />mortal", hintHeader, name _player];
		player setVariable ["hintTimer", 3];
	};
};
...
JJAM_admin_vehGod = {
	...
	if (_vehicle isEqualTo objNull) then {
		...
		if (isDamageAllowed _vehicle) then {
			...
			hintSilent parseText format ["%1You made<br /><t color=""#42ebf4"">%2''s</t><br /> vehicle <t color=""#42ebf4"">immortal</t>", hintHeader, _player];
			player setVariable ["hintTimer", 3];
		} else {
			[objectParent _player, true] remoteExec ["allowDamage", 2];
			hintSilent parseText format ["%1You made<br /><t color=""#42ebf4"">%2''s</t><br /> vehicle <t color=""#42ebf4"">mortal</t>", hintHeader, _player];
			player setVariable ["hintTimer", 3];
		};
	};
};
...
JJAM_admin_freeze = {
	...
	if (_player == player) then {
		hintSilent parseText format ["%1You tried to freeze yourself!<br />Don''t worry I saved you", hintHeader];
		player setVariable ["hintTimer", 3];
	} else {
...
//THIS NEEDS TO BE CHANGED TO WORK ON OTHER PLAYERS (CURRENTLY WILL ALWAYS DELETE YOUR VEHICLE EVEN IF ANOTHER PLAYER IS SELECTED)
...
JJAM_admin_spectator = {
	...
	_player = allPlayers select _selectedIndex;
	if (_player == player) then {
		hintSilent parseText format ["%1You can''t spectate yourself!", hintHeader];
		player setVariable ["hintTimer", 3];
	} else {
...
JJAM_admin_playerInfo = {
	...
	hintSilent parseText format ["%1 %2 <br /> %3 <br /> %4 <br /><br /> <t size=''1.6'' color=''#f45f42''>The players equipment was copied to clipboard, press CTRL+V in a note program to paste it.</t>", hintHeader, _infoName, _infoSteam, _infoUID];
